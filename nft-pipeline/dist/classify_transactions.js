...
exports.classifyAllTransactions = classifyAllTransactions;
exports.getClassificationStats = getClassificationStats;
exports.exportClassificationStats = exportClassificationStats;
 ...
 */
function classifyTransaction(tx, addressRegistry, marketplaceAddresses, buyEntrypoints, config) {
    ...
    if ((0, config_1.isBridgeAddress)(target) || targetInfo?.category === 'bridge') {
        return 'bridge';
    }
    // 4. Check if target is CEX
    if ((0, config_1.isCexAddress)(config, target)) {
        return 'cex_deposit';
    }
    // 5. Check if sender is CEX
    if ((0, config_1.isCexAddress)(config, tx.sender)) {
 ...
 */
function classifyAllTransactions(config, storage) {
    ...
    // Build lookup sets
    const marketplaceAddresses = new Set((0, config_1.getAllMarketplaceAddresses)(config));
    const buyEntrypoints = new Set([
        ...(0, config_1.getAllBuyEntrypoints)(config),
        ...(0, config_1.getAllAcceptOfferEntrypoints)(config)
    ]);
    ...
    for (let startId = 0; startId <= maxId; startId += BATCH_SIZE) {
        const transactions = storage.query(`
      SELECT * FROM all_transactions 
      WHERE id > ? AND id <= ? 
      ORDER BY id
    `, [startId, startId + BATCH_SIZE]);
        for (const tx of transactions) {
            const category = classifyTransaction(tx, addressRegistry, marketplaceAddresses, buyEntrypoints, config);
            // Update category
 ...
 */
function getClassificationStats(storage) {
 ...
 */
function exportClassificationStats(storage, outputPath) {
...
//# sourceMappingURL=classify_transactions.js.map