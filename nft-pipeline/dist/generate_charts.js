 ...
 */
function generateCharts(config, storage) {
    console.log('\n=== Generating Charts ===');
    // Gather data
    ...
    const mpColors = mpLabels.map(label => (0, config_1.isObjktMarketplace)(label) ? '#3B82F6' : // Blue for objkt
        label.includes('teia') ? '#10B981' : // Green for teia
            label.includes('fxhash') ? '#8B5CF6' : // Purple for fxhash
                '#6B7280' // Gray for others
    );
    // Daily fees by objkt vs others
    const objktDailyFees = new Map();
    const otherDailyFees = new Map();
    for (const row of dailyMarketplaceFees) {
        if ((0, config_1.isObjktMarketplace)(row.marketplace)) {
...
//# sourceMappingURL=generate_charts.js.map