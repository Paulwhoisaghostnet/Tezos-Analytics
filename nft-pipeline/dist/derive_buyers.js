...
exports.deriveBuyers = deriveBuyers;
exports.populateBuyerBalances = populateBuyerBalances;
const config_1 = require("./config");
const tzkt_1 = require("./tzkt");
const contract_checker_1 = require("./contract_checker");
 ...
 */
function isValidOpenEdition(tokenContract, marketplace, amount) {
 ...
 */
function isNftTransfer(tokenId, amount, contractAddress, fungibleContracts) {
    // Amount > 5555 = not NFT
 ...
 */
async function deriveBuyers(config, storage) {
    console.log('\n=== Deriving Buyers (with contract verification) ===');
    // Pre-populate known contracts
    (0, contract_checker_1.cacheKnownContracts)(storage);
    // 1. Get all token_id=0 contracts that need checking
    const tokenId0Contracts = (0, contract_checker_1.getTokenId0Contracts)(storage);
    ...
    const contractTypes = await (0, contract_checker_1.batchCheckContracts)(storage, client, tokenId0Contracts);
    // Build set of fungible contracts
    const fungibleContracts = new Set();
    ...
    const buyEntrypoints = (0, config_1.getAllBuyEntrypoints)(config);
    const acceptOfferEntrypoints = (0, config_1.getAllAcceptOfferEntrypoints)(config);
    ...
    const txByTimestampSender = new Map();
    const txByTimestamp = new Map();
    ...
    for (const transfer of allTransfers) {
        // Skip mints
        if (!transfer.from_address || transfer.from_address === '')
            continue;
        // Skip non-wallet recipients
        if (!transfer.to_address || !transfer.to_address.startsWith('tz'))
            continue;
        ...
        if (!isVerifiedSale) {
            ...
            if (matchedTx) {
                const mp = (0, config_1.findMarketplace)(config, matchedTx.target);
                if (mp) {
                    marketplace = mp.name;
        ...
        const isOpenEdition = spendMutez === 0 && isValidOpenEdition(transfer.token_contract, marketplace, spendMutez || 0);
        if (isOpenEdition) {
            openEditionSales++;
        }
 ...
 */
function populateBuyerBalances(storage) {
...
//# sourceMappingURL=derive_buyers.js.map