...
exports.SYNC_WEEKS = void 0;
exports.syncAllData = syncAllData;
exports.syncXtzTransfersOnly = syncXtzTransfersOnly;
exports.syncAllComprehensive = syncAllComprehensive;
exports.initializeWeeklySyncProgress = initializeWeeklySyncProgress;
exports.syncWeek = syncWeek;
exports.printWeeklySyncStatus = printWeeklySyncStatus;
exports.syncAllIncompleteWeeks = syncAllIncompleteWeeks;
 ...
 */
function transferToRaw(transfer) {
    return {
        id: transfer.id,
 ...
 */
async function syncMarketplaceTransactions(config, client, storage, startISO, endISO) {
    ...
    for await (const batch of client.getAllMarketplaceTransactions(marketplaceAddresses, startISO, endISO)) {
        for (const tx of batch) {
            if (lastId !== null && tx.id <= lastId)
                continue;
 ...
 */
async function syncTokenTransfers(config, client, storage, startISO, endISO) {
    ...
    for await (const batch of client.getTokenTransfers(startISO, endISO)) {
        for (const transfer of batch) {
            if (lastId !== null && transfer.id <= lastId)
                continue;
 ...
 */
async function syncBalances(config, client, storage, startISO) {
    ...
    const limit = (0, p_limit_1.default)(config.maxConcurrency);
    let syncedCount = 0;
    let failedCount = 0;
    const promises = addressesNeedingBalance.map(address => limit(async () => {
        try {
 ...
 */
async function syncXtzTransfers(config, client, storage, startISO, endISO) {
    ...
    for (let i = 0; i < addressList.length; i += CHUNK_SIZE) {
        ...
        const promises = chunk.map(address => limit(async () => {
            try {
                ...
                for (const tx of incoming) {
                    ...
                    const transfer = {
                        ...
                        amount: tx.amount,
                        is_from_cex: (0, config_1.isCexAddress)(config, tx.sender?.address || tx.sender),
                        is_to_cex: false
                    };
                ...
                for (const tx of outgoing) {
                    ...
                    const transfer = {
                        ...
                        amount: tx.amount,
                        is_from_cex: false,
                        is_to_cex: (0, config_1.isCexAddress)(config, tx.target?.address || tx.target)
                    };
                    batchBuffer.push(transfer);
                }
 ...
 */
function txToAllTransaction(tx) {
    return {
        id: tx.id,
        hash: tx.hash,
        level: tx.level,
 ...
 */
function txToXtzFlow(tx) {
    const sender = tx.sender.address;
    const target = tx.target?.address || '';
    // Determine flow type based on sender/target
    let flowType = 'p2p';
    if (config_1.KNOWN_CEX_ADDRESSES.includes(sender)) {
        flowType = 'cex_withdrawal';
    }
    else if (config_1.KNOWN_CEX_ADDRESSES.includes(target)) {
        flowType = 'cex_deposit';
    }
    else if ((0, config_1.isBridgeAddress)(target)) {
        flowType = 'bridge_to_l2';
    }
    else if ((0, config_1.isBridgeAddress)(sender)) {
 ...
 */
async function syncAllTransactionsComprehensive(config, client, storage, startISO, endISO) {
 ...
 */
async function syncAllXtzFlows(config, client, storage, startISO, endISO) {
 ...
 */
async function syncAllData(config, storage, includeXtzTransfers = false) {
    ...
    const txCount = await syncMarketplaceTransactions(config, client, storage, window.startISO, window.endISO);
    const transferCount = await syncTokenTransfers(config, client, storage, window.startISO, window.endISO);
    const balanceCount = await syncBalances(config, client, storage, window.startISO);
    ...
    if (includeXtzTransfers) {
        xtzTransferCount = await syncXtzTransfers(config, client, storage, window.startISO, window.endISO);
    }
 ...
 */
async function syncXtzTransfersOnly(config, storage) {
    console.log('='.repeat(60));
    console.log('XTZ TRANSFER SYNC - For CEX Flow Analysis');
    console.log('='.repeat(60));
    const window = (0, config_1.getTimeWindow)(config);
    const client = new tzkt_1.TzktClient(config);
    const count = await syncXtzTransfers(config, client, storage, window.startISO, window.endISO);
    return count;
}
/**
 ...
 */
async function syncAllComprehensive(config, storage) {
    console.log('='.repeat(60));
    ...
    const allTxCount = await syncAllTransactionsComprehensive(config, client, storage, window.startISO, window.endISO);
    // Sync all XTZ flows
    const xtzFlowCount = await syncAllXtzFlows(config, client, storage, window.startISO, window.endISO);
 ...
 */
function initializeWeeklySyncProgress(storage) {
    storage.initializeSyncWeeks(exports.SYNC_WEEKS.map(w => ({ id: w.id, start: w.start, end: w.end })));
}
/**
 * Sync a specific week's comprehensive data
 */
async function syncWeek(config, storage, weekId) {
    const week = exports.SYNC_WEEKS.find(w => w.id === weekId);
    if (!week) {
        throw new Error(`Unknown week: ${weekId}. Valid weeks: ${exports.SYNC_WEEKS.map(w => w.id).join(', ')}`);
    }
    ...
    try {
        ...
        const allTxCount = await syncAllTransactionsComprehensive(config, client, storage, week.start, week.end);
        // Sync all XTZ flows for this week
        console.log('\n--- Syncing XTZ Flows ---');
        const xtzFlowCount = await syncAllXtzFlows(config, client, storage, week.start, week.end);
        // Mark as complete
 ...
 */
function printWeeklySyncStatus(storage) {
    ...
    console.log('');
    for (const week of exports.SYNC_WEEKS) {
 ...
 */
async function syncAllIncompleteWeeks(config, storage) {
    // Initialize weeks if not already done
    initializeWeeklySyncProgress(storage);
