...
exports.deriveWalletFlows = deriveWalletFlows;
exports.exportWalletFlows = exportWalletFlows;
/**
 * Derive wallet XTZ flow summaries from xtz_flows table
 */
function deriveWalletFlows(config, storage) {
    console.log('\n=== Deriving Wallet XTZ Flows ===');
    // Get all unique wallets from xtz_flows
    const wallets = storage.query(`
  ...
  `);
    ...
    const purchases = storage.query(`
    SELECT buyer, seller, spend_mutez, ts FROM purchases WHERE spend_mutez IS NOT NULL
  `);
    // Build lookup maps
    const purchaseByBuyerTime = new Map();
    const saleBySellerTime = new Map();
 ...
 */
function exportWalletFlows(storage, outputPath) {
    const summaries = storage.getAllWalletXtzSummaries();
    const header = [
        'address',
        'balance_start_xtz',
    ...
    const rows = summaries.map(s => {
        const toXtz = (mutez) => mutez !== null ? (mutez / 1_000_000).toFixed(6) : '';
...
//# sourceMappingURL=derive_wallet_flow.js.map