...
exports.deriveResales = deriveResales;
const config_1 = require("./config");
/**
 * Derive resales (secondary sales by collectors) from local data
 */
function deriveResales(config, storage) {
    ...
    if (acceptOfferEntrypoints.length > 0) {
        ...
        for (const collector of buyers) {
            const sellerTxs = storage.getRawTransactionsBySender(collector, marketplaceAddresses, acceptOfferEntrypoints);
            for (const tx of sellerTxs) {
                const marketplace = (0, config_1.findMarketplace)(config, tx.target);
                if (!marketplace)
                    continue;
    ...
    for (const tx of buyTxs) {
        ...
        if (buyerSet.has(tx.sender))
            continue;
        const transfers = storage.getRawTransfersByTransactionId(tx.id);
        for (const transfer of transfers) {
            if (transfer.token_standard !== 'fa2')
                continue;
            const seller = transfer.from_address;
            if (!seller || !buyerSet.has(seller))
                continue;
...
//# sourceMappingURL=derive_resales.js.map