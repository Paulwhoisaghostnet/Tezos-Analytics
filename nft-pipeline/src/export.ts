...
import { calculateVolumeTrend } from './derive_daily_metrics';
import { getMarketplaceBreakdown } from './derive_marketplace_fees';
import { calculateFundFlowSummary } from './derive_fund_flow';

/**
 * Convert mutez to XTZ
 */
function mutezToXtz(mutez: number | null): number | null {
...
export interface SummaryStats {
  ...
  choice_overload_proxy: number; // distinct_tokens_listed / unique_buyers_count
 ...
 */
function calculateSummary(storage: Storage, startISO: string, endISO: string): SummaryStats {
  ...
  // Volume trend
  const volumeTrend = calculateVolumeTrend(storage);
  
  // Marketplace breakdown
  const marketplaceBreakdown = getMarketplaceBreakdown(storage);
  ...
  // CEX flow
  const fundFlow = calculateFundFlowSummary(storage);
...
export function exportData(
  ...
  fs.writeFileSync(
    path.join(outDir, 'buyers.csv'),
    Papa.unparse(buyerRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'buyer_purchases.csv'),
    Papa.unparse(purchaseRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'creators.csv'),
    Papa.unparse(creatorRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'creator_mints.csv'),
    Papa.unparse(mintRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'creator_listings.csv'),
    Papa.unparse(listingRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'creator_offer_accepts.csv'),
    Papa.unparse(offerAcceptRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'collector_resales.csv'),
    Papa.unparse(resaleRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'daily_metrics.csv'),
    Papa.unparse(dailyMetricsRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'marketplace_stats.csv'),
    Papa.unparse(mpStatsRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'daily_marketplace_fees.csv'),
    Papa.unparse(dailyFeesRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'buyer_cex_flow.csv'),
    Papa.unparse(buyerCexRows)
  );
  ...
  fs.writeFileSync(
    path.join(outDir, 'creator_fund_flow.csv'),
    Papa.unparse(creatorFlowRows)
  );
 ...
 */
export function printSummary(summary: SummaryStats): void {
  console.log('\n' + '='.repeat(60));
