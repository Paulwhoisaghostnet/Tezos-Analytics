...
import { PipelineConfig, getAllMarketplaceAddresses, KNOWN_CEX_ADDRESSES } from './config';

// Known contract categories
const KNOWN_CONTRACTS: Record<string, { category: string; alias: string }> = {
  // Etherlink Bridge
...
function getAddressType(address: string): 'wallet' | 'contract' {
...
function isCexAddress(address: string): boolean {
  return KNOWN_CEX_ADDRESSES.includes(address);
}

/**
 * Discover and register top contracts from existing data
 */
export function discoverTopContracts(storage: Storage, limit: number = 50): void {
 ...
 */
export function discoverTopWallets(storage: Storage, limit: number = 200): void {
 ...
 */
export function runAddressDiscovery(storage: Storage, contractLimit: number = 50, walletLimit: number = 200): void {
  console.log('\n' + '='.repeat(60));
 ...
 */
export function exportAddressRegistry(storage: Storage, outputPath: string): void {
  const registry = storage.getAllAddressRegistry();
  
  const header = 'address,address_type,alias,tezos_domain,category,tx_count,resolved_at\n';
  const rows = registry.map(r => 
    `${r.address},${r.address_type},${r.alias || ''},${r.tezos_domain || ''},${r.category || ''},${r.tx_count},${r.resolved_at || ''}`
  ).join('\n');
