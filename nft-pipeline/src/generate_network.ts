...
import { Storage, AddressRegistryRow, XtzFlowRow } from './storage';
...
export interface NetworkNode {
  ...
  totalVolume: number; // Total XTZ transacted
}

// Network edge (transaction link)
export interface NetworkEdge {
  source: string;
  target: string;
  value: number; // Total XTZ amount
  color: string; // Heatmap color
  count: number; // Number of transactions
  avgValue: number; // Average transaction value
}
...
function getHeatmapColor(value: number, min: number, max: number): string {
  if (max === min) return 'rgb(128, 0, 128)'; // Purple for single value
...
function getNodeType(
  address: string,
  registry: Map<string, AddressRegistryRow>,
  config: PipelineConfig
): { type: NetworkNode['type']; category: string | null } {
  const info = registry.get(address);
  
  if (isCexAddress(config, address)) {
    return { type: 'cex', category: 'cex' };
  }
  
  if (isBridgeAddress(address)) {
...
export function generateNetworkData(
  ...
  const registry = new Map<string, AddressRegistryRow>();
  ...
  if (filterWallet) {
    ...
    flows = storage.query<XtzFlowRow>(`
  ...
  const nodeActivity = new Map<string, { count: number; volume: number }>();
  ...
  for (const [, edge] of edgeMap) {
    if (edge.value < minValue) minValue = edge.value;
  ...
  // Build nodes
  const nodes: NetworkNode[] = activeNodes.map(([address, stats]) => {
 ...
 */
export function exportNetworkData(graph: NetworkGraph, outputPath: string): void {
 ...
 */
function generateNetworkHtml(graph: NetworkGraph, filterWallet?: string): string {
